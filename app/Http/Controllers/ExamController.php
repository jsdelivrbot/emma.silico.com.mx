...
class ExamController extends Controller
{
        ...
        public function gradesSpreadshet(Exam $exam)
        {
                $grade = new Grade;
                $board = $exam->board;
                $allGrades = $grade->allStudentsWithData($exam)->toArray();


                        $data = array_map(function ($array) use($allGrades)
                        {
                        ...
                        return Excel::create("CalificacionFinal".$board->shortName(), function ($excel) use($data, $exam, $grade) {
                        $excel->sheet('Primera hoja', function ($sheet) use($data, $exam, $grade) {
                                        $sheet->fromArray($data);
                        });
                        $excel->sheet(
                                'Segunda hoja', function ($sheet) use($data, $exam, $grade) {
                                        $sheet->fromArray(
                                                );
                                });
                })->export('xlsx');
